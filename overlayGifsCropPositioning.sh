...
ffmpegBuildPath="/home/andysnake/ffmpeg"
encoder="" #-c:v h264_nvenc "
outName="output.mp4"
#LOGLEVEL="-loglevel debug"

if [[ ! $OVERLAY_SIZE ]];then OVERLAY_SIZE="1x1";fi #dflt overlay size
#timeStamp="$(date --iso-8601=minutes | perl -pe 's/[^0-9]+//g')"

if [[ $OVERLAY_SIZE == "1x1" ]];then

	$ffmpegBuildPath -y $LOGLEVEL  -hide_banner -i $1 -i $2 -filter_complex " \
		nullsrc=size=$(( $w * 2))x$h [base];[base][0:v] overlay=shortest=1 [tmp1];\
		[tmp1][1:v] overlay=shortest=1:x=$w	"  $encoder $outName

elif [[ $OVERLAY_SIZE == "2x2" ]];then
	if [[ $5 ]];then outName=$5;fi
	$ffmpegBuildPath -y $LOGLEVEL  -hide_banner -i $1 -i $2 -i $3 -i $4  -filter_complex " \
	...
	$ffmpegBuildPath -y $LOGLEVEL -hide_banner -i $1 -i $2 -i $3 -i $4 -i $5 -i $6 -i $7  -i $8 -i $9 -filter_complex " \
